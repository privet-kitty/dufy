...
(defparameter +m-bounds+
  #2A((3.240969941904521d0 -1.537383177570093d0 -0.498610760293d0)
...
the in-gamut colors in CIELUV for a fixed L."
  ...
  (let* ((bounds nil)
         ...
         (sub2 (if (> sub1 +cieluv-epsilon+) sub1 (/ l +cieluv-kappa+))))
    (dotimes (c 3)
      (let ((m1 (aref +m-bounds+ c 0))
            (m2 (aref +m-bounds+ c 1))
            (m3 (aref +m-bounds+ c 2)))
        (dotimes (ti 2)
          (let ((top1 (* sub2 (- (* 284517 m1) (* 94839 m3))))
                (top2 (- (* sub2 l (+ (* 838422 m3) (* 769860 m2) (* 731718 m1)))
                         (* 769860 ti l)))
                (bottom (+ (* sub2 (- (* 632260 m3) (* 126452 m2)))
                           (* ti 126452))))
