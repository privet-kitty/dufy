...
(defun print-make-array (var-name array &key (out t) (declaration t))
  (let ((typ (array-element-type array))
    ...
    (when declaration
      (prin1 `(declaim (type (simple-array ,typ ,dims)
	     ...
	     out)
      (terpri out))
    (format out "(defparameter ~a (load-time-value~% #." var-name)
...
(defun main (obj-filename)
  (let ((obj-path (merge-pathnames obj-filename src-dir-path)))
    (with-open-file (out obj-path
      ...
      (print-make-array "mrd-array-c-h" mrd-array-c-h :out out)
      (print-make-array "mrd-array-c-h-dark" mrd-array-c-h-dark :out out)
      (print-make-array "mrd-array-l" mrd-array-l :out out)
      (print-make-array "mrd-array-l-dark" mrd-array-l-dark :out out)
      (print-make-array "max-chroma-arr" max-chroma-arr :out out)
      (print-make-array "max-chroma-arr-dark" max-chroma-arr-dark :out out))
