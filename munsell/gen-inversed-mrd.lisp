...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (ql:quickload '(:dufy :alexandria :iterate)))

(use-package '(:iterate :dufy/internal))
      ...
      (iter (for lstar from 0 to 100 by 10)
            (iter (for cstarab from 0 to 500 by 20)
                  (multiple-value-bind (hue40 value chroma)
                      (dufy:lchab-to-mhvc-illum-c lstar cstarab hab
                                                  :max-iteration 1000
                                                  :factor 0.4d0
                                                  :threshold 1d-13)
                    (let ((l-idx (/ lstar 10))
...
(main "inversed-munsell-renotation-data.lisp")
