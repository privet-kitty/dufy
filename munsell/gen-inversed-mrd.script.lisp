...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (ql:quickload '(:dufy/core/* :dufy/munsell/invert :alexandria :iterate)))

(use-package '(:iterate :dufy/internal :dufy/munsell/invert))
...
(defun main (dest-filename)
  (let ((dest-path (uiop:merge-pathnames* dest-filename *this-pathname*)))
    (uiop:with-output-file (out dest-path :if-exists :supersede)
      ...
      (format out "~S~%~S~%~%"
              '(uiop:define-package :dufy/munsell/inversed-renotation-data
                (:use :cl)
                (:export #:+inversed-mrd-table-hc+
                 #:+inversed-mrd-table-v+))
              '(in-package :dufy/munsell/inversed-renotation-data))
