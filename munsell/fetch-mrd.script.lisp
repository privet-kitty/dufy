...
;;;
;;; Usage:
;;; $ sbcl --load fetch-mrd.script.lisp
;;;
...
(defun main (dest-filename)
  (let ((dest-path (uiop:merge-pathnames* dest-filename *this-pathname*)))
    (uiop:with-output-file (out dest-path :if-exists :supersede)
      ...
      (format out "~S~%~S~%~%"
              '(uiop:define-package :dufy/munsell/renotation-data
                (:use :cl)
                (:export #:+mrd-table-ch+ #:+mrd-table-ch-dark+
                 #:+mrd-table-l+ #:+mrd-table-l-dark+
                 #:+max-chroma-table+ #:+max-chroma-table-dark+))
              '(in-package :dufy/munsell/renotation-data))
